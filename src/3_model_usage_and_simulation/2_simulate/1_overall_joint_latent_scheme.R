...
joint_latent_structure <- readRDS(paste0("data/4_data_generated/spatial_dependence/", effect_types, "/", model_type, "_cor_mat_ml_estimation_latent_field_matern.rds"))
# Use the raw optimised and not-Matern structure
# joint_latent_structure <- readRDS(paste0("data/4_data_generated/spatial_dependence/", effect_types, "/", model_type, "_cor_mat_ml_estimation_latent_field.rds"))

if(any(is.na(joint_latent_structure))){
  joint_latent_structure[is.na(joint_latent_structure)] <- mean(joint_latent_structure, na.rm = T)
  nearest_corr_mat_optimization <- Matrix::nearPD(joint_latent_structure, corr = TRUE, keepDiag = TRUE)
  joint_latent_structure <- as.matrix(nearest_corr_mat_optimization$mat)
}
...
simulation_results_joint_latent_scheme <- foreach(1:20, .combine = "cbind", .packages = c("tidyverse", "magrittr", "lubridate", "reshape2", "bamlss", "gamlss", "mvtnorm")) %dopar%{
  ...
  simulate(
    ...
    get_rainy_site_idxs_from_positive_probabilities = get_rainy_site_idxs_from_joint_latent_multivariate_standard_gaussian,
...
saveRDS(simulation_results_joint_latent_scheme, paste0("data/4_data_generated/simulation/", effect_types, "/", model_type, "_joint_latent_structure.rds"))
