...
for(season in unique(dat$meteorological_season)){
  
  print(paste0("Calculating season: ", season))
  
  dat_season <- dat %>% filter(meteorological_season == season)
  ...
  for (i in 1:nr_of_sites) {
    for (j in 1:i) {
      if (i == j) {
        print(paste0("Calculations done: ", 100*(i*i - i)/(nr_of_sites*nr_of_sites - nr_of_sites), "%"))
        next
      }
      ...
      temp <- get_pivoted_day_classes_for_sites(dat_season, site1, site2)
  ...
  saveRDS(cor_mat, paste0("data/4_data_generated/spatial_dependence/", effect_types, "/seasonal/", season, "_", model_type, "_cor_mat_ml_estimation_latent_field.rds"))
  saveRDS(ll_vals, paste0("data/4_data_generated/spatial_dependence/", effect_types, "/seasonal/", season, "_", model_type, "_ll_vals_ml_estimation_latent_field.rds"))
  saveRDS(n_datapoints_mat, paste0("data/4_data_generated/spatial_dependence/", effect_types, "/seasonal/", season, "_", model_type, "_n_datapoints_mat_ml_estimation_latent_field.rds"))
  ...
  saveRDS(cor_mat_matern, paste0("data/4_data_generated/spatial_dependence/", effect_types, "/seasonal/", season, "_", model_type, "_cor_mat_ml_estimation_latent_field_matern.rds"))
