...
results_filename <- "src/2_model_selection/2_nonparametric_model_selection/results_occurrence_1_1.csv"

# Boostrap indices
bootstrapped_dataset_ids <- as.matrix(read.table("src/2_model_selection/2_nonparametric_model_selection/0_bootstrap/bootstrapped_dataset_ids.txt"))
...
get_bernoulli_ll <- function(values, prob) {
  return(
    sum(
      log(
        dbinom(values, size = 1, prob = prob)
      )
    )
  )
}
...
calc_evaluation_on_one_bootstrap_sample <- function(i, formula, bootstrapped_dataset_ids, dat) {
  bootstrapped_model <- bamlss(
    formula = formula,
    family = binomial_bamlss,
    data = dat[bootstrapped_dataset_ids[, i], ],
    sampler = FALSE
  )
  
  predictions_real_data <- predict(bootstrapped_model, newdata = dat, type = "parameter")
  ...
  return(list(
    ...
    ll_bootstrap_model_real_data = get_bernoulli_ll(
      values = dat$precip_occurrence,
      prob = predictions_real_data
    )
  ))
}
# 5. Evaluate models ------------------------------------------------------
...
for (temp_i_model in 1:n_models) {
  ...
  ll_real_model_real_data <- temp_model$model.stats$optimizer$logPost
